...
namespace
{ 
	const int ITEM_TYPE_MUSHROOM = 3;
}
...
bool MushroomEnemy::Start()
{
	...
	m_mushroom.SetPosition(m_transform.m_position);	
	m_player = Player::GetInstance();
	m_inGameUI = FindGO<InGameUI>("inGameUI");
	m_mushroomController.Init(25.0f, 80.0f, m_transform.m_position);
...
void MushroomEnemy::Update()
{
	...
	Tach(*m_mushroomCollisionObject, m_mushroomController,false, ITEM_TYPE_MUSHROOM);
	Move();
...
void MushroomEnemy::Move()
{
	...
	Atk();
	m_mushroomCollisionObject->SetPosition(m_transform.m_position);
	m_mushroomCollisionObject->SetRotation(Quaternion::Identity);
	m_mushroomCollisionObject->Update();
	m_transform.m_position = m_mushroomController.Execute(m_transform.m_moveSpeed, g_gameTime->GetFrameDeltaTime());
	m_mushroom.SetPosition(m_transform.m_position);
	m_mushroom.Update();
	Rotation();
...
void MushroomEnemy::Rotation()
{
	...
	toPlayer.Normalize();
	m_transform.m_rotation.SetRotationYFromDirectionXZ(toPlayer);// YŽ²‰ñ‚è‚Ì‰ñ“]‚ðÝ’è
	m_mushroom.SetRotation(m_transform.m_rotation);
}
...
void MushroomEnemy::Atk()
{
	...
	Tach(*m_mushroomCollisionObject, m_mushroomController, true,ITEM_TYPE_MUSHROOM);
}

bool MushroomEnemy::CanAtk() {
	bool CoolTimeFrag = GetDamageCoolTimeFrag();
	if (CoolTimeFrag)
	{
		int coolTime = GetTime();
		coolTime -= g_gameTime->GetFrameDeltaTime();
		SetTime(coolTime);
		if (GetTime() <= 0.0f)
		{
			SetTime(0.0f);
			SetDamageCoolTimeFrag(false);
		}
		else
		{
			return false;
