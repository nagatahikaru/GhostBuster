...
#include "src/Manager/SnowBallManager/SnowBallManager.h"
#include "src/Actor/Character/Enemy/Enemy.h"
#include "src/UI/InGameUI/InGameUI.h"

namespace 
{
	const int ITEM_TYPE_SNOWBOLL = 1;//ドロップアイテムの種類:雪玉
}
...
bool SnowEnemy::Start()
{
	...
	for (int i = 0; i < 2; i++)
	{
		m_snow[i].SetPosition(m_transform.m_position);
	}
	m_snowController.Init(25.0f, 80.0f, m_transform.m_position);
...
{
	m_snow[0].SetPosition(m_transform.m_position);
	m_snow[1].SetPosition(m_transform.m_position);
	SetEnemyHp(10);
	m_form = 0;
...
void SnowEnemy::Update()
{
	...
	Move();	
	Tach(*m_snowCollisionObject, m_snowController,false, ITEM_TYPE_SNOWBOLL);
...
void SnowEnemy::Move()
{
	...
	{
		m_transform.m_moveSpeed.y -= 200.0f;
	}
	else
	{
		int jaumTime = GetTime();
	...
	if (m_player->m_position.y > m_transform.m_position.y + 100&& GetTime() <=0&&m_form==1)
	{
		m_transform.m_moveSpeed.y += 1000.0f;
		SetTime(4.0f);
	}
	//プレイヤーとの距離が一定以下の場合
	//擬態を解いて攻撃をする
	...
	if (m_form == 0)
	{		
		m_transform.m_moveSpeed.x = 0;
		m_transform.m_moveSpeed.z = 0;
	}
	if (distance >= 2000.0f)
	{
		m_form = 0;
	...
	m_snowCollisionObject->SetPosition(m_transform.m_position);
	m_snowCollisionObject->SetRotation(Quaternion::Identity);
	m_snowCollisionObject->Update();
	m_transform.m_position = m_snowController.Execute(m_transform.m_moveSpeed, g_gameTime->GetFrameDeltaTime());
	m_snow[m_form].SetPosition(m_transform.m_position);
...
void SnowEnemy::Rotation()
{
	...
	m_snow[m_form].SetRotation(m_transform.m_rotation);
}
...
{	
	...
	// 敵の位置(少し前にずらすと自然)
	Vector3 pos = m_transform.m_position + dir * 10.0f + Vector3(0.0f, 10.0f, 0);

	// 発射命令
	m_snowBallManager->Fire(pos, dir, rot);
	SetTime(3.5f);
	SetDamageCoolTimeFrag(true);
}
